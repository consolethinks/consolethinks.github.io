...
class weapon_heaterpipe : ScriptBasePlayerWeaponEntity
{
	...
	{
		self.Precache();
		g_EntityFuncs.SetModel( self, self.GetW_Model( "models/poke646/weapons/heaterpipe/w_heaterpipe.mdl") );
		self.m_iClip			= -1;
	...
	void Precache()
	{
		...
		g_Game.PrecacheModel( "models/poke646/weapons/heaterpipe/v_heaterpipe.mdl" );
		g_Game.PrecacheModel( "models/poke646/weapons/heaterpipe/w_heaterpipe.mdl" );
		g_Game.PrecacheModel( "models/poke646/weapons/heaterpipe/p_heaterpipe.mdl" );

		g_SoundSystem.PrecacheSound( "poke646/weapons/heaterpipe/heaterpipe_hit1.wav" );
		g_SoundSystem.PrecacheSound( "poke646/weapons/heaterpipe/heaterpipe_hit2.wav" );
		g_SoundSystem.PrecacheSound( "poke646/weapons/heaterpipe/heaterpipe_miss.wav" );
		...
		g_SoundSystem.PrecacheSound( "poke646/weapons/heaterpipe/heaterpipe_breathe.wav" ); // Fatigue sound
	}
	...
	bool Deploy()
	{
		return self.DefaultDeploy( self.GetV_Model( "models/poke646/weapons/heaterpipe/v_heaterpipe.mdl" ), self.GetP_Model( "models/poke646/weapons/heaterpipe/p_heaterpipe.mdl" ), PIPE_DRAW, "crowbar" );
	}
	...
	void Holster( int skiplocal /* = 0 */ )
	{
		...
		if (m_bIsBreathing)
		{
			g_SoundSystem.StopSound(m_pPlayer.edict(), CHAN_STATIC, "poke646/weapons/heaterpipe/heaterpipe_breathe.wav", false);
	...
	bool Swing( int fFirst )
	{
		...
		else if( m_iSwingCount == 7 )
		{
			...
			{
				m_bIsBreathing = true;
				g_SoundSystem.PlaySound(m_pPlayer.edict(), CHAN_STATIC, "poke646/weapons/heaterpipe/heaterpipe_breathe.wav", 0.65f, 1.0f, 0, 100, m_pPlayer.entindex());
			}
		}
		...
		if ( tr.flFraction >= 1.0 )
		{
			if( fFirst != 0 )
			{
				...
				g_SoundSystem.EmitSoundDyn( m_pPlayer.edict(), CHAN_WEAPON, "poke646/weapons/heaterpipe/heaterpipe_miss.wav", 1, ATTN_NORM, 0, 94 + Math.RandomLong( 0,0xF ) );
	...
	// end aone
			...
			if( fHitWorld == true )
			{
				...
				{
				case 0:
					g_SoundSystem.EmitSoundDyn( m_pPlayer.edict(), CHAN_WEAPON, "poke646/weapons/heaterpipe/heaterpipe_hit1.wav", fvolbar, ATTN_NORM, 0, 98 + Math.RandomLong( 0, 3 ) ); 
					break;
				case 1:
					g_SoundSystem.EmitSoundDyn( m_pPlayer.edict(), CHAN_WEAPON, "poke646/weapons/heaterpipe/heaterpipe_hit2.wav", fvolbar, ATTN_NORM, 0, 98 + Math.RandomLong( 0, 3 ) ); 
					break;
				}
			}
	...
	void WeaponIdle()
	{
		...
		if ( m_bIsBreathing == true && m_iSwingCount == 5 )
		{
			g_SoundSystem.StopSound(m_pPlayer.edict(), CHAN_STATIC, "poke646/weapons/heaterpipe/heaterpipe_breathe.wav", false);
			m_bIsBreathing = false;
		}
...
string GetHeaterpipeName()
{
	return "weapon_heaterpipe";
}

void RegisterHeaterpipe()
{
	g_CustomEntityFuncs.RegisterCustomEntity( "weapon_heaterpipe", GetHeaterpipeName() );
	g_ItemRegistry.RegisterWeapon( GetHeaterpipeName(), "poke646" );
}
